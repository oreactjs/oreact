 ...
 * like import Typography from '@oreact/core/Typography' are tree-shakeable.
 *
...
async function addLicense(packageData) {
    const license = `/** @license Oreact v${packageData.version}
 *
...
`;
    await Promise.all(
        [
            './index.js',
            './esm/index.js',
            './umd/oreact.development.js',
            './umd/oreact.production.min.js',
        ].map(async (file) => {
            try {
...
async function run() {
    try {
        ...
        await Promise.all(
            [
                // use enhanced readme from workspace root for `@oreact/core`
                packageData.name === '@oreact/core' ? '../../README.md' : './README.md',
                '../../CHANGELOG.md',
                '../../LICENSE',
