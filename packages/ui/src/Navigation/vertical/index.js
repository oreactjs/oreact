        ...
        <List className={clsx("navigation whitespace-no-wrap", className)}>
            {
                navigation.map((item) => (
                    ...
                    <React.Fragment key={item.id}>
                        ...
                        {item.type === 'group' && (
                            <NavigationGroup item={item} nestedLevel={0} active={active} dense={dense}/>
                        )}

                        {item.type === 'collapse' && (
                            <NavigationCollapse item={item} nestedLevel={0} active={active} dense={dense}/>
                        )}

                        {item.type === 'item' && (
                            <NavigationItem item={item} nestedLevel={0} active={active} dense={dense}/>
                        )}

                        {item.type === 'link' && (
                            <NavigationLink item={item} nestedLevel={0} active={active} dense={dense}/>
                        )}
